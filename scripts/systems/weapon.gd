...
## Player weapon system with constrained shooting arc.
##
## Holds a list of [WeaponDefinition]s and fires the currently equipped one.
## The weapon rotation is clamped to +/-45 degrees from the player's facing
## direction. Guide lines show the shooting arc boundaries.
##
...
func _ready() -> void:
	...
	if weapon_defs.is_empty():
		...
		_current_def.bullet_scene = preload("res://scenes/props/Bullet.tscn")
		_current_def.max_ammo = 12
		_current_def.damage = 10
		_current_def.fire_rate = 0.25
		_current_def.reload_time = 1.5
		_current_def.bullet_speed = 800.0
		_current_def.bullet_max_range = 200.0
		_current_def.muzzle_offset = Vector2(16, 0)
